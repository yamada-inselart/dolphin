...
var Dolphin = (function() {
  ...
  var Scroll,
		...
		'handleEvents': function() {
			...
			Dolphin.on('animationStop', function() {
				...
				if(self.state === 1) {
					time = new Date() - AccessTime,
					time = U.convertTime(time);
					playTime = new Date() - start;
					playTime = U.convertTime(playTime);
					self.pushTimeline({
						'id': $_selector,
						'event': '停止',
						'time': time,
	...
	AnalysisCore.prototype = {
		'initialize': function() {
			this.userID = CookieModule.getID(),
			...
			this.accessTime = U.convertDate(AccessTime);
