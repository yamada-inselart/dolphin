...
Copyright © 2017 inselart. All rights reserved.
	...
	*/
	...
	Ajax.prototype = {
		...
		'ajaxDebug': function(args) {
			var $_result = document.getElementById('js-result');
			if(this.Dolphin.debug === 0) {
				return;
			}
			switch (args.type) {
			  case 'load':
			    $_result.innerHTML = args.url + 'が読み込み完了しました。';
			    break;
			  case 'error':
			    $_result.innerHTML = 'aJax通信に失敗しました。';
			    break;
			  case 'success':
			    ...
			    break;
			  case 'timeout':
			    $_result.innerHTML = '回線速度が遅いため終了しました。';
			    break;
			}
		}
	}
	...
	Dolphin.prototype = {
		'initialize': function(selector, param) {
			...
			self.$_window = window,
			self.$_selector = document.getElementById(selector),
			self.position = getOffsetTop(self.$_selector);
		...
		'handleEvents': function() {
			...
			self.$_window.addEventListener('load', function() {
				self.position = getOffsetTop(self.$_selector);
			});
			self.$_window.addEventListener('resize', function() {
				self.position = getOffsetTop(self.$_selector);
			});
		},
		'debugMode': function() {
			if(this.debug) {
				var parent = this.$_selector.parentNode,
						resultArea = document.createElement('div');
						resultArea.innerHTML = '動画読み込み中です。';
						resultArea.id = 'js-result';
						resultArea.className = 'result';

				parent.appendChild(resultArea);
			}
		},
		'getState': function() {
		...
		'animateTimer': function(event, t) { // tは経過時間
			...
			function step() {
				self.target.setAttribute('src', animeData.data[self.loop_count].d);
		...
		'scrollPlay': function(dir, event) { //要素が画面内に入ったらスルロールで再生する命令
			...
			//初期設定
			function init() {
				wH = self.$_window.innerHeight,
				sH = self.$_selector.clientHeight,
			...
			function handleEvents() {
				...
				self.$_window.addEventListener('scroll', function() {
					scroll = document.documentElement.scrollTop || document.body.scrollTop,
					scrollEvent();
				});
			}
